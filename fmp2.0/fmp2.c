/*************************************************************************
	> 文件名: FMp3Play v2.0
	> 作者：千城真人
	> 简介: win下的调用SDL2_mixer的mpg123库播放mp3,命令行下执行.
	> 功能：可以单次和循环播放mp3
	> 初版时间：20171012
...
int fmp_play(char* mpfile)
{
	...
	Mix_Music *music = Mix_LoadMUS(localeToUTF8(mpfile));//gbk转utf8让SDL2识别
	Mix_PlayMusic(music,1);//值为-1循环，1为单次
		
	while (Mix_PlayingMusic());	//只要有音乐没读取完就不停止,这句while有水平，才学来的
